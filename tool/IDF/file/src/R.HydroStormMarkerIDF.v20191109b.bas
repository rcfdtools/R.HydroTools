...
Sub R_HydroStormMarker()
    ...
    vHojaIDFEcuacionVal = "IDFEcuacionVal"
    vHojaSetup = "Setup"
    ...
    If vAnswer = vbYes Then
        ...
        Sheets(vHojaIDFValores).Range("B8:ZZ1048576").ClearContents
        Sheets(vHojaIDFEcuacionVal).Range("B8:ZZ1048576").ClearContents
        ...
        If Range(vCeldaIDFCluster) = "SI" Then
            ...
            For i = 0 To vMaxNumPulsos - 1
                ...
                Sheets(vHojaIDFEcuacionVal).Cells(vFilaRotulo, 3 + i) = (i * Range(vCeldaIntervalo)) + Range(vCeldaIntervalo)
            Next i
        End If
        ...
        If Range(vCeldaIDFCluster) = "SI" Then
            ...
            For iAux = 0 To vRegistrosTr - 1
                ...
                Sheets(vHojaIDFEcuacionVal).Cells(vFilaInicio + iAux, 2) = Sheets(vHojaSetup).Cells(4 + iAux, 14)
            Next iAux
            For iAux = 0 To vMaxNumPulsos - 1
            ...
            Range("E4") = vFilaFin + 8
            For iAux = 0 To vMaxNumPulsos - 1
                If Cells(vFilaFin + 2, 3 + iAux) > 0 Then
        ...
        End If
        'IDF - Series de valores obtenidos para la construcción de la ecuación característica a partir de correlación múltiple
        If Sheets(vHojaDatos).Range(vCeldaIDFCluster) = "SI" Then
            For iAux = 0 To vMaxNumPulsos - 1
                For iAux1 = 0 To vRegistrosTr - 1
                    If Sheets(vHojaIDFValores).Cells(vFilaInicio + iAux1, 3 + iAux) > 0 Then 'Intensidad > 0
                ...
                Next iAux1
            Next iAux
            Sheets(vHojaIDFSerie).Select
            Range("D5:F5").Select
            i = Sheets(vHojaIDFSerie).Range("F9").End(xlDown).Row
            Selection.FormulaArray = "=LINEST(R9C6:R" & i & "C6,R9C7:R" & i & "C8,TRUE,FALSE)"
        End If
        'IDF - Valores obtenidos a partir de la ecuación característica
        For iAux = 0 To vMaxNumPulsos - 1
            For iAux1 = 0 To vRegistrosTr - 1
                vdCell = "R" & (vFilaRotulo) & "C" & (3 + iAux)
                vTrCell = "R" & (vFilaInicio + iAux1) & "C2"
                'MsgBox vTrCell & "/" & vdCell
                Sheets(vHojaIDFEcuacionVal).Cells(vFilaInicio + iAux1, 3 + iAux).Formula = "=(R5C5*" & vTrCell & "^R3C5)/(" & vdCell & "^R4C5)"
            Next iAux1
        Next iAux
