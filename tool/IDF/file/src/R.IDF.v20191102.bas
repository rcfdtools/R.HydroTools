...
Sub R_HydroStormMarker()
    ...
    If vAnswer = vbYes Then
        ...
        'MARCACIÓN DE LA COLUMNA DE FRECUENCIAS ACUMULADAS POR TORMENTA
        ...
        For i = vFilaInicio To vFilaFin - 1
            If Cells(i, vColTormentaNum) = Cells(i + 1, vColTormentaNum) Then
                Cells(i + 1, vColFrecAcum) = vIntervalo
                vIntervalo = vIntervalo + Range(vCeldaIntervalo)
            Else
                If vIntervalo > vIntervaloMax Then vIntervaloMax = vIntervalo 'Evaluación de la máxima frecuencia acumulada encontrada
                If Cells(i + 1, vColDatoAcumulado) = 0 And Cells(i + 2, vColDatoAcumulado) = 0 Then
                    vIntervalo = 0
                Else
                    Cells(i + 1, vColFrecAcum) = 0
                    vIntervalo = 10
                End If
            End If
        Next i
        ...
        If Range(vCeldaIDFCluster) = "SI" Then
            ...
            Next i
        End If
        'Delta 1 inicial (corresponde al mismo valor del dato original)
         If Range(vCeldaIDFCluster) = "SI" Then
            For i = vFilaInicio To vFilaFin - 1
                iAux = 0
                If Cells(i, vColTormentaNum) = Cells(i + 1, vColTormentaNum) Then
                ...
                For i = vFilaInicio To vFilaFin - 1
                    ...
                    For iAux1 = 0 To iAux - 1
                        If Cells(i + iAux1, vColTormentaNum) = vTormentaActual Then
                            vCluster = vCluster + Cells(i + iAux1 + 2, vColIDFCluster)
                            iAux2 = iAux2 + 1
                        End If
                    Next iAux1
                    If iAux2 = iAux And Cells(i + iAux + 1, vColTormentaNum) = vTormentaActual Then
