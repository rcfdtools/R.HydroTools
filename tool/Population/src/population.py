...
population_col = 'PRural' # ● PTotal, PUrban, PRural
projection_year_max = 2040 # ● Projection year
...
print(f'* (Art) Arithmetical: Year diff. {t_end} - {t_0} = {t_end-t_0}, Population diff. {p_end} - {p_0} = {p_end-p_0}, Annual growth {annual_growth}')
arithmetical_projection = np.round(p_0+((x_future-t_0)*annual_growth), decimals=0)
df_projected[f'{population_col}Art'] =  arithmetical_projection
if set_negative_to_zero: arithmetical_projection[arithmetical_projection < 0] = 0
...
if show_plot:
    ...
    plt.plot(x_future, arithmetical_projection, color='orange', linestyle='-.', lw=1, label=f'Arithmetical ({int(arithmetical_projection[-1])})')
