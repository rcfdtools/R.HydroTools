...
df_category['CategId'] = ''
#df_category['CategName'] = df_category.index  # Copy stations category to a new column before the category index change
df_category.update(df_category_dict.set_index([category_name]))
#df_category.reset_index()
#df_category.rename(columns = {'CategName':category_name[:10]}, inplace = True)
#df_category = df_category.dropna()
#df_category.to_csv('../.datasets/category_count.csv', header=True, index=None)
df_category.to_csv('../.datasets/category_count.csv', header=True)
print('\n%s' % df_category)
print('\n> This analysis includes only the stations located over the land areas from the hydrographic subzones and tide metering stations could be displayed because not updated locations or because some maritim limits was traced with low tide levels.')